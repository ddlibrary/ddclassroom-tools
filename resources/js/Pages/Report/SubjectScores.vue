<template>
    <Head title="Subject Scores Report" />

    <AuthenticatedLayout>
        <!-- Subject Scores Report -->
        <div class="mb-2">
            <div class="flex justify-between items-center mb-4">
                <h1 class="text-2xl font-bold">Subject Scores Report</h1>
                <a v-if="form.subject_id" :href="exportUrl" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
        ...
        <div v-else class="mb-4">
            <div class="bg-yellow-50 border border-yellow-200 rounded-md p-4">
                <p class="text-yellow-800">Please select a subject to view scores.</p>
            </div>
        </div>

        <no-record-found v-if="form.subject_id && results.data.length == 0"></no-record-found>

        <!-- Pagination -->
        <div v-if="form.subject_id && results.links.length > 3" class="mt-2">
...
const props = defineProps(['results', 'grades', 'subGrades', 'years', 'countries', 'subjects', 'passedCount', 'failedCount']);
...
const exportUrl = computed(() => {
    ...
    return route('reports.subject-scores.export') + '?' + params.toString();
});
...
const goToPage = (url) => {
    ...
    router.get(route('reports.subject-scores'), finalParams, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const search = () => {
    ...
    router.get(route('reports.subject-scores'), params, {
        preserveState: true,
        preserveScroll: true,
    });
};
